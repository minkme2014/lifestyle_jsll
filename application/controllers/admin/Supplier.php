...
class Supplier extends Admin_Controller
{
    ...
    public function saved_supplier($id = NULL)
    {
        ...
        if (!empty($created) && !empty($edited)) {
            ...
            $data = $this->supplier_model->array_from_post(array('name', 'mobile', 'phone', 'email', 'address', 'state', 'city'));
            ...
            if (!empty($check_items)) { // if input data already exist show error alert
                ...
                ### INSERT DATA INTO Payment Term ###
    			if(!empty($return_id) && !empty($_POST['days'][0])){				
    				for($i=0; $i<sizeof($_POST['days']); $i++){
    					$dataPay = array( 'supplier_id' => $return_id, 'days' => $_POST['days'][$i], 'discount' => $_POST['discount'][$i], 'interest' => $_POST['interest'][$i] );
    					$this->db->insert('supplier_payment_term',$dataPay);
    				}
    			}
        ...
        if (!empty($state_id)) {
            ...
            if ($query->num_rows() > 0) {
                foreach ($query->result() as $service) {
                    $option .= "<option value=\"$service->name\">$service->name</option>";
					$count = $service->name;
                } 
                $data['message'] = $option;
                $data['status'] = true;
                ...
                $data['message'] = 'no_service_available';
                $data['status'] = false;
            }
        } else {
            $data['message'] = 'invalid_department';
            $data['status'] = null;
        }
