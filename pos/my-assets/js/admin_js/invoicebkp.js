function addInputField(t) {
    if (count == limits) alert("You have reached the limit of adding " + count + " inputs");
        ...
        " <td>"+
        "<input type='text' name='available_quantity[]' id='' class='form-control text-right available_quantity_" + count + "' value='' readonly='readonly' /></td>"+
        "<td>"+
        ...
        "<td>"+
        "<input type='text' name='product_rate[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ");' value='' id='price_item_" + count + "' class='price_item"+count+" form-control text-right' required tabindex='"+tab3+"'/></td>"+
		...
		"</label><br>"+
		"CGST-<input type='text' id='cgst_" + count + "' class='cgst_" + count + "' name='cgst[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ")' style='width:10%;border:none;'>"+
		"SGST-<input type='text' id='sgst_" + count + "' class='sgst_" + count + "' name='sgst[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ")' style='width:10%;border:none;'>"+
		"IGST-<input type='text' id='igst_" + count + "' class='igst_" + count + "' name='igst[]' onkeyup='quantity_calculate(" + count + ");' onchange='quantity_calculate(" + count + ")' style='width:10%;border:none;'></td>"+
        "<td class='text-right'>"+
        "<input class='total_price form-control text-right' type='text' name='total_price[]' id='total_price_" + count + "' value='' readonly='readonly'/></td><td><input type='hidden' id='total_tax_" + count + "' class='total_tax_" + count + "' />"+
        "<input type='hidden' id='all_tax_" + count + "' class=' total_tax'/><input type='hidden' id='total_discount_" + count + "' class='total_tax_" + count + "' />"+
...
function calculateSum() {
 //   debugger;
  ...
  //  $("#total_tax_ammount").val(a.toFixed(2)), 
	...
	delivery=parseFloat(delivery_charges);
    freight_charges=$("#freight_charges").val();
    if(freight_charges=="")
    {
        freight_charges=0;
    }
	freight=parseFloat(freight_charges);
    ...
    $("#grandTotal").val(+o + +e-f + +delivery + +freight);
    var paid_amount=$("#paidAmount").val();
    if(paid_amount=="")
    {
        paid_amount=0;
    }
    $("#dueAmmount").val((+o + +e-f + +delivery + +freight)-paid_amount);
