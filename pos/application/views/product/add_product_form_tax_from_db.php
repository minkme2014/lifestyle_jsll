                                     ...
                                     <!--   <td class="text-center" width="16%">
										...
										<?php
											  foreach ($taxes as $tax) { ?>
													<!--option value="<?php echo $tax->tax_name . '|' . $tax->tax_per;?>" data-taxrate="<?php echo $tax->tax_per;?>"><?php echo $tax->tax_name.' - '.$tax->tax_per . '%';?>
													</option-->
                                            <?php echo $tax->tax_name; ?> <input type="number" tabindex="9" class="form-control" id="<?php echo strtolower($tax->tax_name); ?>" name="<?php echo strtolower($tax->tax_name); ?>" value="<?php echo $tax->tax_per; ?>" step="0.01" onkeyup="cal_total();" onblur="cal_total();" placeholder="<?php echo strtolower($tax->tax_name); ?>" style="width:45%; display:inline;padding:0;"/><br>
                                               <?php } ?>
...
/* 	if(isNaN($('#cgst').val()) || $('#cgst').val() == "" || $('#cgst').val() == 0){
	...
	<?php 
		foreach ($taxes as $tax) {
			?>
			if(isNaN($("#<?php echo strtolower($tax->tax_name)?>").val()) || $("#<?php echo strtolower($tax->tax_name)?>").val() == "" || $("#<?php echo strtolower($tax->tax_name)?>").val() == 0){
				tax = parseFloat(tax) +  0 ;
			}else{
				tax = parseFloat(tax) + parseFloat($("#<?php echo strtolower($tax->tax_name)?>").val());
			}
	<?php	}
	?>
...
function cal_total(){
	...
	<?php 
		foreach ($taxes as $tax) {
			?>
			if(isNaN($("#<?php echo strtolower($tax->tax_name)?>").val()) || $("#<?php echo strtolower($tax->tax_name)?>").val() == "" || $("#<?php echo strtolower($tax->tax_name)?>").val() == 0){
				//var <?php echo strtolower($tax->tax_name)?> = 0 ;
				total_tax_per = parseFloat(total_tax_per) +  0 ;
			}else{
				//var <?php echo strtolower($tax->tax_name)?> = $("#"+<?php echo strtolower($tax->tax_name)?>).val();
				total_tax_per = parseFloat(total_tax_per) + parseFloat($("#<?php echo strtolower($tax->tax_name)?>").val());
			}
			//total_tax_per = total_tax_per + $("#"+<?php echo strtolower($tax->tax_name)?>);
	<?php	}
	?>
/* 	if(isNaN($('#cgst').val()) || $('#cgst').val() == "" || $('#cgst').val() == 0){
		var cgst = 0 ;
