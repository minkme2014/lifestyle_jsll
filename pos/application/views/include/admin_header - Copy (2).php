...
//echo "<h>aESRDTFGYH<\h>";
    function buildMenu($parent, $menu, $sub = NULL)
    {
		$CI = & get_instance();
		$user_email = $CI->session->userdata('user_email'); 
	//	print_r($CI->uri->segment()) ;

         $html = "";
        $ul_class = "";
         if (!empty(config_item('layout-h'))) {
            $ul_class = 'horizontal_menu navbar-nav';
           // $ul_class = 'sidebar_menu';
        } 
		//return "here";
        if (isset($menu['parents'][$parent])) {
            if (!empty($sub)) {
                $html .= "<ul id=" . $sub . " class='nav s-menu sidebar-subnav collapse'><li style='display:none' class=\"sidebar-subnav-header\"><a href=". $menu['items'][$itemId]->link.">". display($sub) . "</a></li>\n";
           //     $html .= "<ul class='treeview-menu'><li><a href=". $menu['items'][$itemId]->link.">" . display($sub) . "</a></li>\n";
            } else {
            ...
            foreach ($menu['parents'][$parent] as $itemId) {
                ...
                if (!isset($menu['parents'][$itemId])) { //if condition is false only view menu
					...
					if($menu['items'][$itemId]->label=="manage_online_invoice")
					{
						if($user_email=="spiink2018@gmail.com"){
							
							   $html .= "<li class='" . $active . "' >\n  <a $terget title='" . display($menu['items'][$itemId]->label) . "' href='" . base_url() . $menu['items'][$itemId]->link . "'>\n <em class='" . $menu['items'][$itemId]->icon . "'></em> <span>" . display($menu['items'][$itemId]->label) . "</span></a>\n</li> \n";
                 ...
                 //   $html .= "<li class='" . $active . "' >\n  <a $terget title='" . display($menu['items'][$itemId]->label) . "' href='" . base_url() . $menu['items'][$itemId]->link . "'>\n <em class='" . $menu['items'][$itemId]->icon . "'></em> <span>" . display($menu['items'][$itemId]->label) . "</span></a>\n</li> \n";
                 //   $html .= "<li class='treeview '>\n  <a title='" . display($menu['items'][$itemId]->label) . "' href='" . base_url() . $menu['items'][$itemId]->link . "'>\n <em class='" . $menu['items'][$itemId]->icon . "'></em> <span>" . display($menu['items'][$itemId]->label) . "</span></a>\n</li> \n";
				}
					if (isset($menu['parents'][$itemId])) { //if condition is true show with submenu
						$html .= "<li class='sub-menu " . $active . "'>\n  <a data-toggle='collapse' href='#" . $menu['items'][$itemId]->label . "'> <em class='" . $menu['items'][$itemId]->icon . "'></em> <span>" . display($menu['items'][$itemId]->label) . "</span><span class='pull-right-container'><i class='fa fa-angle-left pull-right'></i></span></a>\n";
					//    $html .= "<li class='treeview'>\n <a href='#'><i class='".$menu['items'][$itemId]->icon."'></i> <span>" . display($menu['items'][$itemId]->label) . "</span><span class='pull-right-container'><i class='fa fa-angle-left pull-right'></i></span></a>";
						$html .= buildMenu($itemId, $menu, $menu['items'][$itemId]->label);
						$html .= "</li> \n";
					}
				}
            $html .= "</ul> \n";
...
<aside class="main-sidebar">
    ...
    <div class="sidebar">
        ...
        if ($user_type != 1) {// query for employee user role
            $CI->db->select('tbl_user_role_permission.*', FALSE);
            $CI->db->select('tbl_menu.*', FALSE);
            $CI->db->from('tbl_user_role_permission');
            $CI->db->join('tbl_menu', 'tbl_user_role_permission.menu_id = tbl_menu.menu_id', 'left');
            $CI->db->where('tbl_user_role_permission.usr_role_id', $user_type);
            ...
            $user_menu = $CI->db->where('status', 1)->order_by('sort', 'time','parent')->get('tbl_menu')->result();
        }
        ...
        <ul class="sidebar-menu">
            ...
            ?>">
                ...
                <ul class="treeview-menu">
                    ...
                    <?php if($this->session->userdata('user_email')=='spiink2018@gmail.com'){ ?>
                    <li><a href="<?php echo base_url('Cinvoice/manage_online_invoice') ?>"><?php echo display('manage_online_invoice') ?></a></li>
                    <?php } ?>
                    <li><a href="<?php echo base_url('Cinvoice/pos_invoice') ?>"><?php echo display('pos_invoice') ?></a></li>
                    <li><a href="<?php echo base_url('Cinvoice/odr_manufacturing_costing') ?>"><?php echo display('odr_manufacturing_costing') ?></a></li>
                    <li><a href="<?php echo base_url('Cinvoice/sales_record') ?>"><?php echo display('sales_record') ?></a></li>
                    <li><a href="<?php echo base_url('Cinvoice/manufacturing_costing') ?>"><?php echo display('manufacturing_costing') ?></a></li>
                </ul>
            </li-->
            <!-- Invoice menu end -->
            <!-- Category menu start -->
            ...
            ?>">
                ...
                <ul class="treeview-menu">
                    ...
                    <li><a href="<?php echo base_url('Cproduct/manage_rol') ?>"><?php echo display('rol_product') ?></a></li>
                    <?php
            ...
            <!-- Activity menu start -->
            <!--li class="treeview <?php
            if ($this->uri->segment('1') == ("Cactivity")) {
                echo "active";
            } else {
                echo " ";
            }
            ?>">
                <a href="#">
                    <i class="fa fa-tag"></i><span><?php echo display('activities') ?></span>
                    <span class="pull-right-container">
                        <i class="fa fa-angle-left pull-right"></i>
                    </span>
                </a>
                <ul class="treeview-menu">
                    <li><a href="<?php echo base_url('Cactivity') ?>"><?php echo display('add_activity') ?></a></li>
                    <li><a href="<?php echo base_url('Cactivity/manage_activity') ?>"><?php echo display('manage_activity') ?></a></li>
                </ul>
            </li-->
            <!-- Activity menu end -->
        ...
        if ($this->session->userdata('user_type') == '1') {
            ...
            ?>">
                ...
                <ul class="treeview-menu">
                    ...
                    <li><a href="<?php echo base_url('Admin_dashboard/todays_commission_report') ?>"><?php echo display('commission_report') ?></a></li>
                    <li><a href="<?php echo base_url('Admin_dashboard/odr_manufacturing_report') ?>"><?php echo display('odr_manufacturing_report') ?></a></li>
                    <li><a href="<?php echo base_url('Admin_dashboard/profit_sheet') ?>"><?php echo display('profit_sheet') ?></a></li>
                    <li><a href="<?php echo base_url('Admin_dashboard/sales_record_report') ?>"><?php echo display('sales_record_report') ?></a></li>
                    <li><a href="<?php echo base_url('Admin_dashboard/manufacturing_cost_report') ?>"><?php echo display('manufacturing_cost_report') ?></a></li>
                </ul>
            </li-->
            <!-- Report menu end -->
        ...
        <!--<li><a href="<?php echo base_url('data_synchronizer/synchronize/') ?>"><?php echo display('synchronize') ?></a></li>-->
            ...
            ?>">
                ...
                <ul class="treeview-menu">
                    ...
                    <li><a href="<?php echo base_url('User/role') ?>"><?php echo display('add_role') ?></a></li>
                    <li><a href="<?php echo base_url('User/manage_role') ?>"><?php echo display('manage_role') ?></a></li>
