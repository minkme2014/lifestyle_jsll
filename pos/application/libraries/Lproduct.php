...
class Lproduct {
    ...
    public function product_list($links, $per_page, $page) {
        ...
        $CI->load->model('Userm');
		$permission = $CI->Userm->get_permission("manage_product");
	...
	public function product_rol($links, $per_page, $page) {
        $CI = & get_instance();
        $CI->load->model('Products');
        $CI->load->model('Web_settings');
        $CI->load->model('Userm');
		$permission = $CI->Userm->get_permission("rol_product");
        $products_list = $CI->Products->product_rol($per_page, $page);
        $all_product_list = $CI->Products->all_product_list_rol();
        ...
        $productList = $CI->parser->parse('product/rol', $data, true);
        return $productList;
    }
    ...
    public function product_add_form() {
        ...
        $CI->load->model('Userm');
		$permission = $CI->Userm->get_permission("add_product");
...
//        echo '<pre>';        print_r($category_selected);die();
        ...
        $data = array(
            ...
            'utgst' => $product_detail[0]['utgst'],
            'total' => $product_detail[0]['total'],
            'uom_selected' => $product_detail[0]['uom'],
            'total_sell_price' => $product_detail[0]['total_sell_price'],
            'type' => $product_detail[0]['type'],
            'code' => $product_detail[0]['code'],
            'rack' => $product_detail[0]['rack'],
            'uom_list' => $uom_list,
        );
