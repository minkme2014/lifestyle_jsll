...
class Admin_dashboard extends CI_Controller {
    ...
    public function index() {
        ...
        if (!$this->auth->is_logged()) {
            redirect('/Admin_dashboard/do_login/', 'refresh');
			//$this->output->set_header("Location: " . base_url() . 'Admin_dashboard/do_login', TRUE, 302);
        }
    ...
    public function all_reports() {
        ...
        $content = $CI->parser->parse('include/all_reports', $data, true);
        $this->template->full_admin_html_view($content);
    }
 ...
 /*        if ($this->session->userdata('user_type') == '2') {
    ...
    public function todays_consumption_report() {
     /*    if ($this->session->userdata('user_type') == '2') {
        ...
        $CI->load->library('lreport');
        $this->auth->check_admin_auth();
        $content = $CI->lreport->todays_consumption_report();
        $this->template->full_admin_html_view($content);
    }

    #==============Todays_commission_report============#
   ...
   /*      if ($this->session->userdata('user_type') == '2') {
        ...
        $CI->load->library('lreport');
        $this->auth->check_admin_auth();
        $content = $CI->lreport->todays_commission_report();
        $this->template->full_admin_html_view($content);
    }
 ...
 /*        if ($this->session->userdata('user_type') == '2') {
    ...
    #============Date wise consumption report==============#

    public function retrieve_dateWise_ConsumptionReports() {
      /*   if ($this->session->userdata('user_type') == '2') {
            $this->session->set_userdata(array('error_message' => display('you_are_not_access_this_part')));
            redirect('Admin_dashboard');
        } */
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->retrieve_dateWise_ConsumptionReports($from_date, $to_date);
        $this->template->full_admin_html_view($content);
    }

    #============Date wise commission report==============#

    public function retrieve_dateWise_CommissionReports() {
     /*    if ($this->session->userdata('user_type') == '2') {
            $this->session->set_userdata(array('error_message' => display('you_are_not_access_this_part')));
            redirect('Admin_dashboard');
        } */
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $emp_id = $this->input->post('sales_prsn');
        $content = $CI->lreport->retrieve_dateWise_CommissionReports($from_date, $to_date,$emp_id);
        $this->template->full_admin_html_view($content);
    }
   ...
   /*      if ($this->session->userdata('user_type') == '2') {
	...
	/* 	if(empty($category_id))
		{
			echo "here";
			//$config["total_rows"] = $this->Reports->retrieve_product_search_sales_report_count($from_date, $to_date, $category_id);
		} */
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        ...
        #pagination ends
        #  
        $content = $this->lreport->get_products_search_report($from_date, $to_date, $category_id, $links, $config["per_page"], $page);
    ...
    public function do_login() {
        ...
        /*if ($setting_detail[0]['captcha'] == 0 && $setting_detail[0]['secret_key'] != null && $setting_detail[0]['site_key'] != null) {
            ...
            if ($error != '') {
				...
				header("Location: http://$server_name/demo/");
	...
	public function odr_manufacturing_report()
	{	
    /*     if ($this->session->userdata('user_type') == '2') {
            $this->session->set_userdata(array('error_message' => display('you_are_not_access_this_part')));
            redirect('Admin_dashboard');
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->odr_manufacturing_report($from_date, $to_date);
        $this->template->full_admin_html_view($content);
	}
	public function profit_sheet()
	{	
    /*     if ($this->session->userdata('user_type') == '2') {
            $this->session->set_userdata(array('error_message' => display('you_are_not_access_this_part')));
            redirect('Admin_dashboard');
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->profit_sheet($from_date, $to_date);
        $this->template->full_admin_html_view($content);
	}
	public function edit_embroidery_cost()
	{	
        $CI = & get_instance();
        $this->auth->check_admin_auth();
		$invoice=$_POST['invoice'];
		$data=array(
		'embroidery_cost'=>$_POST['embroidery_cost'],
		);
        $CI->load->model('Reports');
        $content = $CI->Reports->edit_embroidery_cost($data,$invoice);
		if($content==true)
		{
	...
	public function sales_record_report()
	{	
      /*   if ($this->session->userdata('user_type') == '2') {
            $this->session->set_userdata(array('error_message' => display('you_are_not_access_this_part')));
            redirect('Admin_dashboard');
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->sales_record_report($from_date, $to_date);
        $this->template->full_admin_html_view($content);
	}
		
	public function manufacturing_cost_report()
	{	
      /*   if ($this->session->userdata('user_type') == '2') {
            $this->session->set_userdata(array('error_message' => display('you_are_not_access_this_part')));
            redirect('Admin_dashboard');
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->manufacturing_cost_report($from_date, $to_date);
        $this->template->full_admin_html_view($content);
	}
	...
	{	
        ...
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->purchase_return_report($from_date, $to_date);
        $this->template->full_admin_html_view($content);
	}
		
	public function invoice_return_report()
	{	
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lreport');
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');
        $content = $CI->lreport->invoice_return_report($from_date, $to_date);
        $this->template->full_admin_html_view($content);
	}
